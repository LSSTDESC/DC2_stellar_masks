...
import sys
import call_dc2
#from s_multiprocessing import _Multiprocessing
...
class Calculate_density:
    ...
    def _surface_trapeze(self):
        ...
        ra_galaxies, dec_galaxies = np.array(tract_galaxies["ra"]), np.array(tract_galaxies["dec"])
        dec_min, dec_max = min(dec_galaxies), max(dec_galaxies)
        condition_B = (abs(dec_galaxies)>=abs(dec_min)-0.2) & (abs(dec_galaxies)<=abs(dec_min)+0.2) # Find the small side
        condition_b = (abs(dec_galaxies)>=abs(dec_max)-0.1) & (abs(dec_galaxies)<=abs(dec_max)+0.1) # Find the larger side
        ra_min_b, ra_max_b = min(ra_galaxies[condition_b]), max(ra_galaxies[condition_b]) 
        ra_min_B, ra_max_B = min(ra_galaxies[condition_B]), max(ra_galaxies[condition_B])
        small_side = ra_max_b - ra_min_b
        large_side = ra_max_B - ra_min_B
        mean_side = (large_side+small_side)/2
        delta_dec = dec_max - dec_min
        center_dec = (dec_min + dec_max)/2 # Because we are on a sphere, surface needs to be corrected with dec position
        tract_surface = mean_side*delta_dec*np.sin(np.radians(90-center_dec))*(3600**2)
    ...
    def get_density_around_stars(self, theta_bins = np.logspace(np.log10(0.5),np.log10(50), 50)):
        ...
        condition = np.array([(abs(self.galaxies_tract_neighbours["ra"] - self.stars["ra"][i])*3600 <= max(theta_bins)) | (abs(self.galaxies_tract_neighbours["dec"] - self.stars["dec"][i])*3600 <= max(theta_bins)) for i in range(len(self.stars["ra"]))])
        sum_density = 0
        for i, star in enumerate(self.stars):
            ra_galaxies, dec_galaxies = self.galaxies_tract_neighbours["ra"][condition[i]], self.galaxies_tract_neighbours["dec"][condition[i]] # Restrain number of galaxies for which we compute distance
            distance = self._distance(ra_galaxies, dec_galaxies, star["ra"], star["dec"]) # Distance between star and galaxies within ra or dec max theta
            number_galaxies = np.array([len(distance[distance<=theta_bins[j]]) for j in range(len(theta_bins))]) #Number of objects within distance theta
            surface = self._surface_circle(theta = theta_bins, dec = star['dec'])
            sum_density += number_galaxies/surface # true density for each theta_bin will then be density / len(stars)
...
class Critical_radius:
    def __init__(self, name='dc2_object_run2.2i_dr6_v2_with_addons_v2', theta_bins = np.logspace(np.log10(0.5),np.log10(50), 50), tract_list = None, quantities = ['ra','dec','mag_i_cModel'], conditions=None, conditions_galaxies=["extendedness==1", "mag_i_cModel>17", "mag_i_cModel<25.3"], conditions_stars=["extendedness==0"], binned_quantity="mag_i_cModel", bins=[0, 17, 18, 20, 22, 24]):
        ...
        if tract_list is None:
            self.tract_list = [5074, 5073, 5072, 5071, 5070, 5069, 5068, 5067, 5066, 5065, 4860, 4859, 4858, 4857, 4856, 4855, 4854, 4853, 4852, 4851, 
                4850, 4648, 4647, 4646, 4645, 4644, 4643, 4642, 4641, 4640, 4639, 4638, 4637, 4636, 4441, 4440, 4439, 4438, 4437, 4436, 
                4435, 4434, 4433, 4432, 4431, 4430, 4429, 4236, 4235, 4234, 4233, 4232, 4231, 4230, 4229, 4228, 4227, 4226, 4225, 4224, 
                4035, 4034, 4033, 4032, 4031, 4030, 4029, 4028, 4027, 4026, 4025, 4024, 4023, 3837, 3836, 3835, 3834, 3833, 3832, 3831, 
                3830, 3829, 3828, 3827, 3826, 3825, 3643, 3642, 3641, 3640, 3639, 3638, 3637, 3636, 3635, 3634, 3633, 3632, 3631, 3453, 
                3452, 3451, 3450, 3449, 3448, 3447, 3446, 3445, 3444, 3443, 3442, 3441, 3268, 3267, 3266, 3265, 3264, 3263, 3262, 3261, 
                3260, 3259, 3258, 3257, 3256, 3086, 3085, 3084, 3083, 3082, 3081, 3080, 3079, 3078, 3077, 3076, 3075, 3074, 2908, 2907, 
    ...
    def get_density_ratio(self, tract):
